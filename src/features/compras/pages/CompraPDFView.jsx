  ...
  return (
    <div style={{ backgroundColor: '#f3f4f6', minHeight: '100vh', padding: '32px 16px' }}>
      ...
      <div style={{ maxWidth: 800, margin: '0 auto 20px auto', display: 'flex', gap: 12, justifyContent: 'flex-end' }}>
        <button className="crud-btn crud-btn-secondary" onClick={() => navigate(`/admin/compras/detalle/${id}`)}>
          ← Volver
        </button>
      ...
      <div
        ref={contentRef}
        style={{
          maxWidth: 800, margin: '0 auto', backgroundColor: '#ffffff',
          padding: '48px 56px', borderRadius: 8,
          boxShadow: '0 4px 24px rgba(0,0,0,0.08)',
          fontFamily: 'Roboto, Arial, sans-serif', color: '#111', fontSize: '0.95rem',
        }}
      >
        {/* Encabezado */}
        ...
        <div style={{
          ...
          backgroundColor: '#f9fafb', padding: '16px 20px', borderRadius: 6, border: '1px solid #e5e7eb',
        }}>
          <div>
          ...
          <div>
            ...
            <p style={{ margin: 0, fontWeight: 600, color: compra.estado === 'Completada' ? '#16a34a' : '#dc2626' }}>
              {compra.estado}
            </p>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
          <div style={{ width: 260, backgroundColor: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: 8, padding: '16px 20px', fontSize: '0.95rem' }}>
            {[
              ...
              <div key={label} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8, color: '#6b7280' }}>
                <span>{label}</span><span>{formatCurrency(value || 0)}</span>
              </div>
            ))}
            <div style={{
              display: 'flex', justifyContent: 'space-between',
              borderTop: '2px solid #e5e7eb', paddingTop: 10, marginTop: 4,
              fontWeight: 700, fontSize: '1.05rem', color: 'var(--primary-color)',
            }}>
              <span>TOTAL</span><span>{formatCurrency(compra.total || 0)}</span>
            </div>
          </div>
        </div>
