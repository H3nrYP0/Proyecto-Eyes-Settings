...
const ProductDetail = ({ user }) => {
  ...
  if (isLoading) {
    return (
      <div className="pd-page">
        <div className="pd-loading"><LoadingSpinner mensaje="Cargando producto…" /></div>
      </div>
    );
  }
    ...
    return (
      <div className="pd-page">
        <div className="pd-error-state">
          ...
          <p>{error?.message || "Este producto no existe o fue eliminado."}</p>
  ...
  const {
    nombre = "Producto", descripcion = "", precioVenta = 0,
    ...
    <div className="pd-page">
      ...
      <div className="pd-layout">
              ...
              {imagenesFiltradas.map((img, idx) => (
                ...
                >
                  {imgErrors[idx] ? (
                    ...
                    <img src={img.url} alt={`Vista ${idx + 1}`} onError={() => handleImgError(idx)} />
                  )}
                </button>
              ))}
            </div>
        ...
        <div className="pd-info">
          {(marca || categoria) && (
            <p className="pd-meta">{[marca, categoria].filter(Boolean).join(" · ")}</p>
          )}
          <h1 className="pd-nombre">{nombre}</h1>
          {codigo && <p className="pd-codigo">Ref: {codigo}</p>}
          ...
          <div className="pd-disponibilidad">
            {!disponible ? (
              <span className="pd-stock pd-stock--out"><span className="pd-stock-dot" />Agotado</span>
            ) : stockBajo ? (
              <span className="pd-stock pd-stock--low"><span className="pd-stock-dot" />Últimas {stockActual} unidades</span>
            ) : (
              <span className="pd-stock pd-stock--in"><span className="pd-stock-dot" />En stock ({stockActual} disponibles)</span>
            )}
          </div>
