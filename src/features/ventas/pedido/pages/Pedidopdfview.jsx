...
export default function PedidoPDFView() {
  ...
  useEffect(() => {
    const cargar = async () => {
      try {
        ...
        setPedido(data);
        const info = await pedidosService.getInfoAbonos(Number(id), data.total ?? 0);
  ...
  const itemsProducto = (pedido.items ?? []).filter((i) => i.tipo === "producto");
  const itemsServicio = (pedido.items ?? []).filter((i) => i.tipo === "servicio");
        ...
        <tbody>
          {items.map((item, i) => (
            <tr key={i} style={{ backgroundColor: i % 2 === 0 ? "#fff" : "#f9fafb" }}>
              <td style={{ border: "1px solid #e5e7eb", padding: "8px 12px" }}>{item.nombre}</td>
              <td style={{ border: "1px solid #e5e7eb", padding: "8px 12px", textAlign: "right" }}>
                {item.cantidad}
              </td>
              <td style={{ border: "1px solid #e5e7eb", padding: "8px 12px", textAlign: "right" }}>
                {formatCurrency(item.precio)}
              </td>
              <td style={{ border: "1px solid #e5e7eb", padding: "8px 12px", textAlign: "right", fontWeight: 600 }}>
  ...
  return (
    <div style={{ backgroundColor: "#f3f4f6", minHeight: "100vh", padding: "32px 16px" }}>
      {/* Botones */}
      <div style={{ maxWidth: 800, margin: "0 auto 20px auto", display: "flex", gap: 12, justifyContent: "flex-end" }}>
        <button className="crud-btn crud-btn-secondary" onClick={() => navigate(-1)}>
          ← Volver
        </button>
        <button className="crud-btn crud-btn-primary" onClick={handleDescargar}>
          ⬇ Descargar PDF
        </button>
      </div>
      ...
      >
        ...
        <div style={{ textAlign: "center", marginBottom: 32, borderBottom: "2px solid #e5e7eb", paddingBottom: 20 }}>
          ...
          <p style={{ margin: "16px 0 4px", fontWeight: 700, fontSize: "1.1rem", textTransform: "uppercase", letterSpacing: 1 }}>
            Soporte de Pedido
          </p>
        ...
        <div style={{
          ...
          {[
            ...
            ["Estado",            pedido.estado?.charAt(0).toUpperCase() + (pedido.estado?.slice(1) ?? ""), estadoColor.color],
            ...
            <div key={label}>
              ...
              <p style={{ margin: 0, fontWeight: 600, textTransform: "capitalize", color: color || "#111" }}>
                {value}
              </p>
            </div>
          ))}
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 8 }}>
          <div style={{
            width: 290, backgroundColor: "#f9fafb",
            border: "1px solid #e5e7eb", borderRadius: 8,
            padding: "16px 20px", fontSize: "0.95rem",
          }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
              <span style={{ color: "#6b7280" }}>Total</span>
              <span style={{ fontWeight: 700 }}>{formatCurrency(pedido.total)}</span>
            </div>

            {abonos.length > 0 && (
              <>
                <div style={{ borderTop: "1px solid #e5e7eb", marginTop: 8, paddingTop: 8 }}>
                  <p style={{ margin: "0 0 6px", fontWeight: 600, fontSize: "0.82rem", color: "#6b7280", textTransform: "uppercase" }}>
                    Historial de abonos
                  </p>
                  {abonos.map((a, i) => (
                    <div key={a.id} style={{ display: "flex", justifyContent: "space-between", marginBottom: 4, fontSize: "0.85rem" }}>
                ...
                <div style={{
                  ...
                  borderTop: "2px solid #e5e7eb", paddingTop: 10, marginTop: 8,
                  fontWeight: 700, fontSize: "1rem",
                  color: saldoPendiente <= 0 ? "#10b981" : "#ef4444",
                }}>
                  <span>Saldo pendiente</span>
                  <span>{saldoPendiente <= 0 ? "✓ Pagado" : formatCurrency(saldoPendiente)}</span>
                </div>
              </>
            )}
          </div>
        ...
        {pedido.transferencia_comprobante && (
          <div style={{ marginTop: 28, padding: "14px 18px", backgroundColor: "#f9fafb", borderRadius: 6, border: "1px solid #e5e7eb" }}>
            <p style={{ margin: "0 0 4px", fontWeight: 600, color: "#374151" }}>Comprobante de transferencia</p>
