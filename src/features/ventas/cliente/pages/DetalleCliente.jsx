  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      if (status === 401 || status === 422) {
        ...
        alert(err?.response?.data?.error ?? "Error al guardar la fórmula.");
      }
    } finally {
      setSaving(false);
    }
  ...
  return (
    ...
    >
      <div style={{
        ...
        {authError && (
          <div style={{
            ...
            <div style={{ fontSize: "0.88rem" }}>
              ...
              Por favor <strong>cierra sesión e inicia sesión nuevamente</strong> para acceder al historial de fórmulas.
            </div>
            <button
              onClick={() => {
        ...
        {/* Contenido normal — solo si no hay error de auth */}
        {!authError && (
          <>
            {!showForm && (
              <button onClick={() => setShowForm(true)} className="crud-btn crud-btn-primary" style={{ marginBottom: 16 }}>
                + Nueva Fórmula
              </button>
            )}
